...
#define PAGE_READONLY           0x02 // file must have GENERIC_READ
#define PAGE_READWRITE          0x04 // file must have GENERIC_READ and GENERIC_WRITE
#define PAGE_WRITECOPY          0x08 // equivalent to PAGE_READONLY
#define PAGE_EXECUTE_READ       0x20 // XPsp2 or Server2003sp1 or later, file must have GENERIC_READ and GENERIC_EXECUTE
#define PAGE_EXECUTE_READWRITE  0x40 // XPsp2 or Server2003sp1 or later, file must have GENERIC_READ, GENERIC_WRITE and GENERIC_EXECUTE
#define PAGE_EXECUTE_WRITECOPY  0x80 // Vista Sp1 or later
#define SEC_COMMIT         0x1000000 // map as an executable (including integrity checks) (i.e page protection defined by the file itself)
#define SEC_COMMIT_NO_EXECUTE 0x11000000 // Win8 or later. map as executable, but do not do integrity checks
extern "C" HANDLE __stdcall CreateFileMappingW(HANDLE hFile, void* psa, UINT page_prot, UINT cbMaxHigh, UINT cbMaxLow, const wchar_t * name);
extern "C" HANDLE __stdcall CreateFileMappingA(HANDLE hFile, void* psa, UINT page_prot, UINT cbMaxHigh, UINT cbMaxLow, const char * name);
...
extern "C" void * __stdcall MapViewOfFile(HANDLE hFileMap, UINT access, UINT cbOffsetHigh, UINT cbOffsetLow, size_t cbToMap);
extern "C" BOOL __stdcall UnmapViewOfFile(const void * pvBase);
...
c* append_num(c* psz, int ii, int min_width = 1, c lead = ' ')
{
   ...
   return return append_num(psz, (unsigned int)(ii), false, min_width, lead);
}
